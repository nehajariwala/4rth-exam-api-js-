...
const uimaker = (data) => {
    ...
    data.map((ele, i) => {
        ...
        let ratingg = document.createElement("p")
        ratingg.innerHTML = ele.rating.count
       
      

        let div = document.createElement("div")
        div.append(image,category, title, price, description, rating,ratingg)
...
const search=(value)=>{
    let temp=products.filter((ele)=>ele.title==value)                            
    uimaker(temp)
  }
  const handalserch=(e)=>{
    e.preventDefault()
